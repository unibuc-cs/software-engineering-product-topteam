...
export default function CourseDetailsPage() {
  ...
  const handleDeleteMeeting = (meetingId: string) => {
    setCourse((prevCourse) => {
      ...
      return {
        ...prevCourse,
        meetings: prevCourse.meetings.filter(
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <h1 className="text-3xl font-bold mb-4">{course.denumire}</h1>
      {isEditing ? (
        <EditCourse
          course={course}
        ...
        <div className="bg-white shadow-md rounded-lg p-6">
          <p className="text-gray-600 mb-4">{course.descriere}</p>
          <div className="flex justify-between items-center mb-4">
            <span className="text-blue-600 font-bold text-xl">
              ${course.pret.toFixed(2)}
            </span>
            <span className="text-gray-500">NrSedinte: {course.nrSedinte}</span>
          </div>
          <p className="text-gray-600">
            Enrolled Students: "To be implemented"
          </p>
          <button
